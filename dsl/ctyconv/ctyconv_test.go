...
func TestJSONToCtyValue(t *testing.T) {
	eventJson, err := os.ReadFile("../testdata/raw_change_event.json")
	assert.NoError(t, err)

	eventCty, err := JSONToCtyValue(eventJson)
	assert.NoError(t, err)
...
func BenchmarkJSONToCty(t *testing.B) {
	eventJson, err := os.ReadFile("../testdata/raw_change_event.json")
	assert.NoError(t, err)

	for n := 0; n < t.N; n++ {
		_, err := JSONToCtyValue(eventJson)
		if err != nil {
			return
		}
	}
...
func TestConvertCtyValueToInterface(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result, err := CtyValueToInterface(tc.input)
...
func TestConvertCtyValueToInterface_CapsuleTypes(t *testing.T) {
	...
	t.Run("capsule Type 1", func(t *testing.T) {
		result, err := CtyValueToInterface(capsuleVal1)
		if assert.NoError(t, err) {
	...
	t.Run("capsule Type 2", func(t *testing.T) {
		result, err := CtyValueToInterface(capsuleVal2)
		if assert.NoError(t, err) {
